#!/usr/bin/env python3
"""
Настройка автоматического деплоя для локального репозитория
"""
...
"""
    ...
    if sys.platform != 'win32':
        run_command(f"chmod +x {hooks_dir}/pre-push")
    
    print("✅ Pre-push hook настроен")
    
    # Post-commit hook для уведомлений
    post_commit_hook = """#!/bin/bash
# Post-commit hook для уведомлений

echo "Post-commit hook: Коммит создан"
...
if telegram_token and admin_chat_id:
    import requests
    message = 'Новый коммит создан в репозитории'
...
"""
    ...
    # Сохраняем post-commit hook
    with open(f"{hooks_dir}/post-commit", 'w') as f:
        f.write(post_commit_hook)
    
    # Делаем исполняемым
    if sys.platform != 'win32':
        run_command(f"chmod +x {hooks_dir}/post-commit")
    
    print("✅ Post-commit hook настроен")
...
"""
    ...
    # Сохраняем скрипт
    with open('auto_commit.sh', 'w') as f:
        f.write(auto_commit_script)
    
    # Делаем исполняемым
    if sys.platform != 'win32':
        run_command('chmod +x auto_commit.sh')
...
"""
    ...
    # Сохраняем скрипт
    with open('start_auto_deploy.sh', 'w') as f:
        f.write(startup_script)
    
    # Делаем исполняемым
    if sys.platform != 'win32':
        run_command('chmod +x start_auto_deploy.sh')
...
def main():
    ...
    try:
        ...
        print("   2. Запустите: ./start_auto_deploy.sh")
        print("   3. Любые изменения автоматически коммитятся и деплоятся!")
        print("\n🛑 Для остановки:")
        print("   kill $(cat file_watcher.pid)")
