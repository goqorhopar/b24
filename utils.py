...
def truncate_text(text: str, max_length: int, suffix: str = "...", 
    ...
    if preserve_words and available_length > max_length * 0.5:
        ...
        if last_space > available_length * 0.7:  # Если пробел не слишком далеко
...
def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        checks['checks']['environment'] = {
            ...
            'required_vars_ok': sum(env_validation['required_vars'].values()),
            'optional_vars_ok': sum(env_validation['optional_vars'].values())
        }
...
def clean_html_tags(text: str, allowed_tags: List[str] = None) -> str:
    ...
    if allowed_tags is None:
        ...
        clean_text = re.sub(f r'<(?!/?(?:{allowed_pattern})\b)[^>]*>', '', text)
