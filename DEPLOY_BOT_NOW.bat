@echo off
echo ========================================
echo 🚀 РАЗВЕРТЫВАНИЕ БОТА НА СЕРВЕРЕ
echo ========================================
echo.
...
ssh -o StrictHostKeyChecking=no root@109.172.47.253 "cd /root/b24 && git pull origin main || echo 'Git pull failed'"
...
ssh -o StrictHostKeyChecking=no root@109.172.47.253 "sleep 10 && systemctl status meeting-bot.service --no-pager"

echo.
echo ========================================
echo 🎉 БОТ РАЗВЕРНУТ НА СЕРВЕРЕ!
echo ========================================
echo 🤖 Бот работает на сервере 109.172.47.253
echo 🔄 Автоматический запуск при загрузке
echo 🔄 Автоперезапуск при сбоях
echo 📱 Отправьте сообщение боту в Telegram!
echo ========================================
